...
class TLDetector(object):
    ...
    def _extract_image(self, pred_image_mask, image):
        ...
        return image[int(top * self.resize_height_ratio):int(
            bottom * self.resize_height_ratio),
               int(left * self.resize_width_ratio):int(
                   right * self.resize_width_ratio)]

    def detect_traffic_light(self, cv_image):
        resize_image = cv2.cvtColor(
            cv2.resize(cv_image, (self.resize_width, self.resize_height)),
            cv2.COLOR_RGB2GRAY)
        ...
        image_mask = \
        self.detector_model.predict(resize_image[None, :, :, :], batch_size=1)[
            0]
    ...
    def process_traffic_lights(self):
        ...
        if self.pose is not None and (self.has_image or self.use_ground_truth):
            ...
            if (tl_id >= 0):
                ...
                wp_id = self.get_closest_waypoint(stop_line_pose,
                                                  self.waypoints)
                rospy.logdebug("[TL_DETECTOR] Closest WP id: %s.", wp_id)

                if (wp_id == -1):
                    rospy.logdebug(
                        "[TL_DETECTOR] Unable to determine valid TL id.")
                    return -1, TrafficLight.UNKNOWN

                closest_wp = self.waypoints[wp_id]
                waypoint_dist = self.dist_to_point(closest_wp.pose.pose,
                                                   tl.pose.pose)
                ...
                if self.use_ground_truth:
                    state = tl.state
                    rospy.loginfo(
                        "[TL_DETECTOR] Using ground-truth information. Nearest TL-state is: %s",
                        state)
                ...
                rospy.logdebug(
                    "[TL_DETECTOR] Upcoming TL ID: %s, State: %s, Distance: %s. ",
                    tl_id, state, car_dist)
                rospy.logdebug(
                    "[TL_DETECTOR] Upcoming TL stop line position: x: %s | y: %s.",
                    stop_line[0],
                    stop_line[1])
                return wp_id, state
