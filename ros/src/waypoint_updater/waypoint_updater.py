...
class WaypointUpdater(object):
    def __init__(self):
        ...
        rospy.Subscriber('/traffic_waypoint', CustomTrafficLight,
                         self.traffic_cb)
    ...
    def do_work(self):
        rate = rospy.Rate(5)
        # ROS parameters
        self.cruise_speed = self.kmph_to_mps(
            rospy.get_param('~/waypoint_loader/velocity', 40.0))
        self.decel_limit = abs(
            rospy.get_param('~/twist_controller/decel_limit', -5))
        ...
        while not rospy.is_shutdown():
            if (
                            self.car_position != None and self.waypoints != None and self.tl_state != None and self.car_curr_vel != None):
                self.safe_distance = (self.car_curr_vel ** 2) / (
                2 * self.decel_limit * SAFE_DECEL_FACTOR)
                self.closestWaypoint = self.NextWaypoint(self.car_position,
                                                         self.car_yaw,
                                                         self.waypoints)
                if self.tl_idx != None:
                    self.distance_to_tl = self.distance(self.waypoints,
                                                        self.closestWaypoint,
                                                        self.tl_idx)
                self.car_action = self.desired_action(self.tl_idx,
                                                      self.tl_state,
                                                      self.closestWaypoint,
                                                      self.waypoints)
                self.generate_final_waypoints(self.closestWaypoint,
                                              self.waypoints, self.car_action,
                                              self.tl_idx)
                self.publish()
            else:
                ...
                if self.tl_idx == None and rand < 0.01:
                    rospy.logwarn(
                        "[WP_UPDATER] /traffic_waypoint not received")
                if self.car_curr_vel == None and rand < 0.01:
                    rospy.logwarn(
                        "[WP_UPDATER] /current_velocity not received")
            rate.sleep()
    ...
    def pose_cb(self, msg):
        ...
        quaternion = (car_orientation.x, car_orientation.y, car_orientation.z,
                      car_orientation.w)
    ...
    def check_stop(self, tl_index, tl_state, closestWaypoint, dist):
        ...
        stop2 = (
        tl_index == closestWaypoint and dist < STOP_DISTANCE and stop0)
        stop3 = (
        tl_index + STOP_HYST > closestWaypoint and stop0 and dist == 99999)
        return stop1 or stop2 or stop3

    def check_slow(self, tl_state, dist):
        slow1 = (
        dist > STOP_DISTANCE and dist < self.safe_distance and tl_state != "NO" and dist != 99999)
        slow2 = (
        dist > STOP_DISTANCE and dist < 2 * STOP_DISTANCE and tl_state != "NO" and dist != 99999 and self.car_curr_vel > self.mph_to_mps(
            3.0))
